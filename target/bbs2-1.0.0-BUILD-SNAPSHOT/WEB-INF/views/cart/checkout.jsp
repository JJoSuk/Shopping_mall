	...
	<div class="container mt-5 ">
		...
		<section class="w-75 ps-5">
				...
				<tbody>
					...
					<c:forEach var="dto" items="${sessionScope.dtos}">
						<tr>
							...
							<td><input type="checkbox" name="chk" value="${dto.cNum}" /></td>
							<td><image
									src="<c:url value="/uploadedFile/${dto.pImage}"/>" width="80px" />
							</td>
							<td>${dto.pName}</td>
							<td>${dto.pQty}</td>
							<td>${dto.price}</td>
					...
					<tr>
						<form action="cartDelete.do" name="cartForm" method="post">
						<input type ="hidden" name="delProdNums"/>
							<td colspan="6">
							<a href="javascript:prodDel()" class="btn btn-sm btn-outline-secondary">선택삭제</a></td>
						</form>
					</tr>
				</tbody>
			</table>
			<div class="text-center">
			<fmt:formatNumber value="${cartTotPrice}"/> 원 <br/>
			<fmt:formatNumber value="${cartTotPoint}"/> 포인트 
			</div>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<label for="use"> 
							전액사용 [<fmt:formatNumber value="${cartTotPoint}" /> 포인트] </label></td>
					</tr>

					<tr>
						<td>결제금액</td>
						<td id="cartTotPrice">
							<fmt:formatNumber value="${cartTotPrice}"/> 원</td>
					</tr>
			...
			<div id="card">
				...
				<br /> 할부기간 : <select <c:if test="${cartTotPrice < 50000}">disabled</c:if>>
			...
			<div class="text-center">
				...
				<a href="" class="btn btn-outline-primary">계속 쇼핑하기</a>
			</div>
		</section>
	</div>
