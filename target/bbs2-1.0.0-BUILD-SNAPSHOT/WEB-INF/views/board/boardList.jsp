...
<%--     <td>${dto.bid}</td>--%>
         <td>${rowNum}</td>
         <td><a href="<c:url value="/board/view.do?bid=${dto.bid}&viewPage=${pDto.viewPage}&cntPerPage=${pDto.cntPerPage}&searchType=${pDto.searchType}&keyWord=${pDto.keyWord}"/>"><c:out value="${dto.subject}"/>
            <b>[<c:out value="${dto.replyCnt}"/>]</b></a></td>
         <td>${dto.hit}</td>
